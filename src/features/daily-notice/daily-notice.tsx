...
export function DailyNotice() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        // "https://notica.rhinolabs.ir/api/ext/events/v1/daily"
  ...
  console.log({ events: data?.result.events });
    ...
    <GlassContainer className="grow flex items-stretch justify-between max-h-[148px] gap-4">
      ...
      {data && data.result.events.length ? (
        <>
          ...
          >
            <p className={"text-lg"}>{data.result.events[0].description}</p>
            <p className={"text-xs"}>
              {data.result.events[1].description} -{" "}
              {data.result.events[2].description}
            </p>
          </div>
        </>
      ) : (
